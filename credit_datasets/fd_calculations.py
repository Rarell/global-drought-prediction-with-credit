...
def calculate_climatology(path_to_data = '../era5'):
    '''
    ...
    test = {}
    # with Dataset('../gldas/evaporation/gldas.evaporation.daily.2004.nc', 'r') as nc:
    with Dataset('../era5/evaporation/evaporation_2000.nc', 'r') as nc:
        test['e'] = nc.variables['evap'][:]
        # test['e'] = nc.variables['e'][:]
    ...
    for year in all_years:
        print(year)
        # e = load_nc('%s/evaporation/gldas.evaporation.daily.%04d.nc'%(path_to_data, year), 'evap')
        # pet = load_nc('%s/potential_evaporation/gldas.potential_evaporation.daily.%04d.nc'%(path_to_data, year), 'pevap')
        e = load_nc('%s/evaporation/evaporation_%04d.nc'%(path_to_data, year), 'e')
    ...
    with Dataset('%s/esr_climatologies.nc'%(path_to_data), 'w', format = 'NETCDF4') as nc:
        nc.description = 'Daily climatology for ESR using daily ERA5 reanalysis data for ET and PET'
        ...
        # nc.createVariable('lat', e['lat'].dtype, ('latitude', ))
        # nc.createVariable('lon', e['lon'].dtype, ('longitude', ))
        nc.createVariable('lat', e['lat'].dtype, ('latitude', 'longitude'))
        nc.createVariable('lon', e['lon'].dtype, ('latitude', 'longitude'))
...
def calculate_sesr(et, pet, year, path_to_data = '../era5'):
    '''
    ...
    # Make the years, months, and day variables
...
def calculate_fdii(smp, year, apply_runmean = True, use_mask = True, path_to_mask = '../era5'):
    '''
    ...
    for i in tqdm(range(I), desc = 'Calculating FD_INT'):
        for j in range(J):
            if use_mask:
                # Ignore sea values
                if np.nansum(np.isnan(smp[:,i,j])) == smp.shape[0]: # For GLDAS2 only
                    continue
...
def calculate_sm_percentiles(year, level = 1, path_to_data = '../era5'):
    '''
    ...
    # Short, nc name of the soil moisture and percentile variables
    # sm_sname = 'soilm' 
    sm_sname = 'swvl%d'%level
    ...
    test = {}
    # with Dataset('%s/%s/gldas.%s.daily.%04d.nc'%(path_to_data, var_name, var_name, year), 'r') as nc:
    with Dataset('%s/liquid_vsm/volumetric_soil_water_layer_%d_%04d.nc'%(path_to_data, level, year), 'r') as nc:
    ...
    T, I, J= test[sm_sname].shape # Also obtain the dataset to intialize the percentile dataset
    ...
    sm = []
    for y in all_years:
        # with Dataset('%s/%s/gldas.%s.daily.%04d.nc'%(path_to_data, var_name, var_name, y), 'r') as nc:
        with Dataset('%s/liquid_vsm/volumetric_soil_water_layer_%d_%04d.nc'%(path_to_data, level, y), 'r') as nc:
    ...
    for i in tqdm(range(I)):
        for j in range(J):
            ...
            #     with Dataset('%s/liquid_vsm/volumetric_soil_water_layer_%d_%04d.nc'%(path_to_data, level, y), 'r') as nc:
            #         sm_time_series.append(nc.variables[sm_sname][:,i,j])
            #         #for t in range(nc.variables[sm_sname].shape[0]):
            #             #sm_time_series.append(nc.variables[sm_sname][t,i,j])
            
            sm_time_series = np.concatenate(sm_time_series)
            # sm_time_series = sm[:,i,j]
            ...
            for t, date in enumerate(dates_year):
                ...
                current_day = np.where((date.day == days) & (date.month == months) & (date.year == years))[0] # Determine what index to use for the current percentile calculation
    ...
    with Dataset('%s/soil_moisture_percentiles_%d_%04d.nc'%(path_to_data, level, year), 'w', format = 'NETCDF4') as nc:
        # nc.description = 'Daily GLDAS2 reanalysis data for percentiles of soil moisture layer %d'%level
        nc.description = 'Daily ERA5 reanalysis data for percentiles of soil moisture layer %d'%level
        ...
        # Create the lat, lon, and time information
        # nc.createVariable('lat', test['lat'].dtype, ('latitude',))
        # nc.createVariable('lon', test['lon'].dtype, ('longitude',))
        nc.createVariable('lat', lat.dtype, ('latitude', 'longitude'))
        nc.createVariable('lon', lon.dtype, ('latitude', 'longitude'))
        nc.createVariable('date', str, ('time'))
