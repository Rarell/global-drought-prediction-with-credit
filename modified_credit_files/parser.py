...
def credit_main_parser(conf, parse_training=True, parse_predict=True, print_summary=False):
    ...
    if parse_training:
        ...
        if "use_regional_latitude_weights" not in conf["loss"]:
            conf["loss"]["use_regional_latitude_weights"] = False
...
def training_data_check(conf, print_summary=False):
    ...
    if len(train_years_range) > 2:
        for year in range(train_years_range[2], train_years_range[3]):
