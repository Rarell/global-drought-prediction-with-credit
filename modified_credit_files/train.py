...
def main(rank, world_size, conf, backend=None, trial=False):
    ...
    #master_port = str(np.random.randint(1000, 8000))

    #print('setting setup with address and port: ', master_addr, master_port)
    if conf["trainer"]["mode"] in ["fsdp", "ddp"]:
        #mp.spawn(setup, args = (world_size, conf["trainer"]["mode"], master_addr, master_port, backend), nprocs = world_size)
