...
if __name__ == "__main__":
    ...
    image_height = int(conf["model"]["image_height"])
    image_width = int(conf["model"]["image_width"])
    conf["model"]["post_conf"]["activate"] = False # Turn off postblock for the summary
    ...
    os.environ["RANK"] = str(torch.cuda.current_device()); os.environ["WORLD_SIZE"] = "1" # COMMENT OUT AFTER TEST RUN; these are not in os environ in the tests
    if conf["trainer"]["mode"] in ["fsdp", "ddp"]:
