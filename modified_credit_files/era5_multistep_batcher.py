    ...
    def load_zarr_as_input(self, i_file, i_init_start, i_init_end, mode="input"):
        ...
        if mode in ["input", "forcing"]:
            ...
            if self.filename_forcing is not None:
                ...
                month_day_forcing = extract_month_day_hour(np.array(sliced_forcing["time"].astype("datetime64[h]")))
                month_day_inputs = extract_month_day_hour(np.array(sliced_x["time"].astype("datetime64[h]")))
                # indices to subset
