    ...
    def __getitem__(self, index):
        ...
        if self.surface_files:
            ...
            ERA5_subset = ERA5_subset.merge(
                surface_subset#, compat = 'override'
            )
        ...
        if self.xarray_forcing:
            ...
            month_day_forcing = extract_month_day_hour(
                np.array(self.xarray_forcing["time"].astype("datetime64[h]"))
            )
            month_day_inputs = extract_month_day_hour(
                np.array(historical_ERA5_images["time"].astype("datetime64[h]"))
            )  # <-- upper air
            # indices to subset
        ...
        sample["datetime"] = [
            int(
                np.datetime64(historical_ERA5_images.time.values[0])
                #.astype("datetime64[s]")
                .astype(int)
            ),
            int(np.datetime64(target_ERA5_images.time.values[0]).astype(int)),
        ]
