'''
In the event MODIS .hdf data is downloaded via other means, this script processes the data without the downloading step
'''
...
def downscale_data(base_fname, variable, sname, modis_sname, timestamp, start_time, day_interval):
    '''
    ...
    for filename in tqdm(filenames, desc = 'Interpolating tiles to grid for %04d-%02d-%02d'%(timestamp.year, timestamp.month, timestamp.day)):
        if variable == 'reflectance': # Note the reflectance has multiple variables to save (multiple reflectance channels) rather than just one
            data_modis = {}
            for n, sn in enumerate(sname):
                data_modis[sn], lat_modis, lon_modis, long_name, units = tile_to_grid(filename, modis_sname[n], path = './')

                I_modis, J_modis = data_modis[sn].shape
        else:
            data_modis, lat_modis, lon_modis, long_name, units = tile_to_grid(filename, modis_sname[0], path = './')
...
if __name__ == '__main__':
    ...
    for year in years: # Need to loop through each year as the timesteps reset to Jan. 1 at the start of each year
        ...
        param_args = [(base_fname, args.variables[0], sname, args.var_snames_modis, timestamp, start_time, args.day_interval) for timestamp in timestamps[::args.day_interval]]
