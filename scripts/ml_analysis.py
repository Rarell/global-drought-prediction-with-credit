...
TODO:
 - Load Data
 - Score card plots for the same
 - SESR and FDII plots
 - SESR and FDII plots where SESR is made from E and PET predictions, and FDII from SM predictions
 - Other metric plots?

 - Start by test running new fd index calculation function for 2-6 rotations (only needs to start process)
 - Test run fd index calculation with single experiment
 - Test run subset metric calculation with single experiment
'''
import gc
...
from plotting import (
    ...
    make_comparison_map,
    make_anomaly_map, 
    make_comparison_subset_map_2, 
    make_anomaly_subset_map_2,
...
# Define all possible subsets
subsets = ['nh', 'sh', 'tropics', 'africa', 'africa_nh', 'africa_sh', 'africa_tropics', 'conus', 'south central']

# Define all upper air and diagnostic variables (surface variables are the ones not included)
...
path_to_rotation = {
    ...
    'single_run': '/ourdisk/hpc/ai2es/sedris/results/one_experiment_run/'
}
...
def make_climatology_and_persistence_metrics(args, timestamp):
    '''
    ...
    clim_filename = 'clim_%04d-%02d-%02dT00Z.nc'%(timestamp.year, timestamp.month, timestamp.day) if args.subset is None else '%s_clim_%04d-%02d-%02dT00Z.nc'%(args.subset, timestamp.year, timestamp.month, timestamp.day)
    persist_filename = 'persist_%04d-%02d-%02dT00Z.nc'%(timestamp.year, timestamp.month, timestamp.day) if args.subset is None else '%s_persist_%04d-%02d-%02dT00Z.nc'%(args.subset, timestamp.year, timestamp.month, timestamp.day)
    print(clim_filename)

    if os.path.exists('%s/climatology/rotation_%02d/%s'%(args.prediction_path, rot, clim_filename)) & os.path.exists('%s/persistence/rotation_%02d/%s'%(args.prediction_path, rot, persist_filename)):
    ...
    for metric in metrics_initial.columns:
        ...
        if var_name == 'Overall':
            ...
            # What zarr files to find the variable in
            ...
            tmp_climatology = load_climatology(var_name, timestamp.month, timestamp.day, 
                                                Ndays = forecast_length, level = level, clim_year = timestamp.year,
                                                type = 'running', path = args.data_path)
            tmp_persistence = load_persistence(var_name, data_type, timestamp.year, timestamp.month, timestamp.day, 
                                                Ndays = forecast_length, level = level, 
                                                type = 'standard', path = args.data_path)

            # Calculate metric for persistence/climatology
            climatology = calculate_metric(tmp_climatology, metric_name, var_name, 
                                            timestamp.year, timestamp.month, timestamp.day, 
                                            data_type, subset = args.subset,
                                            Ndays = forecast_length, apply_mask = args.apply_mask,
                                            level = level, path = args.data_path)
            persistence = calculate_metric(tmp_persistence, metric_name, var_name, 
                                            timestamp.year, timestamp.month, timestamp.day, 
                                            data_type, subset = args.subset,
                                            Ndays = forecast_length, apply_mask = args.apply_mask,
                                            level = level, path = args.data_path)
    ...
    with Dataset('%s/climatology/rotation_%02d/%s'%(args.prediction_path, rot, clim_filename), 'w') as nc:
        ...
        for key in climatology_metrics.keys():
            if len(key) < 5: # length of the key is less than 5 only when the metric name 
                             # (acc, rmse, mse, mae) is used, i.e., for overall metrics, which are None
...
def make_subset_metrics(args, timestamp):
    '''
    '''
    ### Insert portion that does fd index calculations if they exist
    ...
    if fd_indices is True:
        ...
        # Add fd indices to the columns so that they are calculated
    ...
    for metric in metrics_initial.columns:
        ...
        # Get individual information on the metric
        metric_name, var_name, level = get_metric_information(metric)
        ...
        metric_calculation = calculate_metric(tmp, metric_name, var_name,
                                              timestamp.year, timestamp.month, timestamp.day, data_type, 
                                              subset = args.subset, Ndays = forecast_length, apply_mask = args.apply_mask,
                                              level = level, prediction_dataset = 'subset', path = args.data_path)
...
def add_fd_indices(direct, sm, dates_all, one_year, esr_means, esr_stds, mask):
    ...
    smp1 = calculate_sm_percentiles(data['swvl1'], sm[1], dates, dates_all, mask = mask, level = 1)
    smp2 = calculate_sm_percentiles(data['swvl2'], sm[2], dates, dates_all, mask = mask, level = 2)
    smp3 = calculate_sm_percentiles(data['swvl2'], sm[3], dates, dates_all, mask = mask, level = 3)
    smp4 = calculate_sm_percentiles(data['swvl3'], sm[4], dates, dates_all, mask = mask, level = 4)
    ...
    for t, file in enumerate(np.sort(nc_files)):
...
def make_fd_indices(args):
    ...
    # For ERA5 only
    # Load a land-sea mask; using this to skip sea points can save a great deal of time
    ...
    if args.nprocesses > 1:
        # Set parameters
        param_args = [(direct, sm, dates_all, one_year, esr_means, esr_stds, mask) for direct in np.sort(all_direct)]
        # param_args = [(direct) for direct in np.sort(all_direct)]

        # Add FD indices to .nc files with nprocesses
        with mp.Pool(args.nprocesses) as pool:
            data = pool.starmap(add_fd_indices, param_args)
            # data = pool.map(add_fd_indices, param_args)
    else:
        # If only 1 process, add FD indices via loop
        if args.fd_task_number == -1:
            for direct in np.sort(all_direct):
                add_fd_indices(direct, sm, dates_all, one_year, esr_means, esr_stds, mask)
        else:
            ...
            for direct in np.sort(all_direct)[start_ind:end_ind]:
                add_fd_indices(direct, sm, dates_all, one_year, esr_means, esr_stds, mask)

    return

def make_case_study_plots(args, variable, case_study_start, case_study_end, pred_path, verbose = True):
    '''
    '''
    ...
    make_metric_error_plots(y_true, files, times, lat, lon, 
                            sname, plot_sname, args.subset, variable, 
                            args.apply_mask, args.data_path, args.figure_path)
    ...
    for file in new_sort(files):
        ...
        if args.subset is not None:
            ...
            make_comparison_subset_map_2(y, y_pred, clim_pred, lat, lon, time_pred, args.subset,
                                        var_name = plot_sname, forecast_hour = fh,
                                        path = args.figure_path, savename = '%s_%s_forecast_hour_%04d.png'%(args.subset, variable, fh))
            figure_fns = '%s_%s_forecast_hour_*.png'%(args.subset, variable)

            y_anom = y - clim_pred
            y_pred_anom = y_pred - clim_pred
            make_anomaly_subset_map_2(y_anom, y_pred_anom, lat, lon, time_pred, args.subset,
                                    var_name = plot_sname, forecast_hour = fh,
                                    path = args.figure_path, savename = '%s_%s_anomaly_forecast_hour_%04d.png'%(args.subset, variable, fh))
            ...
            make_histogram_scatter_plot(y, y_pred, plot_sname, fh/24, #max_counts = 60,
                                        path = args.figure_path, savename = '%s_%s_2d_histogram_forecast_hour_%04d.png'%(args.subset, variable, fh))
            ...
            make_comparison_map(y, y_pred, clim_pred, lat, lon, time_pred,
                                ...
                                path = args.figure_path, savename = '%s_forecast_hour_%04d.png'%(variable, fh))
            figure_fns = '%s_forecast_hour_*.png'%variable

            y_anom = y - clim_pred
            y_pred_anom = y_pred - clim_pred
            make_anomaly_map(y_anom, y_pred_anom, lat, lon, time_pred,
                             var_name = plot_sname, forecast_hour = fh,
                             path = args.figure_path, savename = '%s_anomaly_forecast_hour_%04d.png'%(variable, fh))
            ...
            make_histogram_scatter_plot(y, y_pred, plot_sname, fh/24, #max_counts = 300,
                                        path = args.figure_path, savename = '%s_2d_histogram_forecast_hour_%04d.png'%(variable, fh))
            histogram_figure_fns = '%s_2d_histogram_forecast_hour_*.png'%(variable)
        
    #  Collect all the newly created figures
    images = []
...
if __name__ == '__main__':
    ...
    # Get all the datetimes available calculations will be done for
    ...
    # Make climatology and persistence metrics?
    ...
    # Similar to make clim_metrics, but for subset of prediction metrics, to go in forecasts/metrics and saved as csv
    ...
    if args.make_performance_plots | args.make_score_cards:
        metric_list, clim_list, persist_list = load_metrics(rotations, all_dates, test_years, 
                                                            subset = args.subset, forecast_length = forecast_length, 
                                                            path_to_data = args.prediction_path, 
                                                            paths_to_rotation = path_to_rotation)

    # Performance plot calculations
    if args.make_performance_plots:
        ...
        # for rot in rotations:
        #     # Determine the path
        #     data_path = '%s/forecasts/metrics'%path_to_rotation[rot]

        #     # Collect all files to load
        #     if args.subset is None:
        #         files = glob('%s/*T00Z.csv'%(data_path), recursive = True)
        #         clim_files = glob('%s/climatology/clim_*.nc'%(args.prediction_path), recursive = True)
        #         persist_files = glob('%s/persistence/persist_*.nc'%(args.prediction_path), recursive = True)
        #     else:
        #         files = glob('%s/*T00Z_%s.csv'%(data_path, args.subset), recursive = True)
        #         clim_files = glob('%s/climatology/%s_clim_*.nc'%(args.prediction_path, args.subset), recursive = True)
        #         persist_files = glob('%s/persistence/%s_persist_*.nc'%(args.prediction_path, args.subset), recursive = True)
    ...
    if args.make_score_cards:
        ...
        # Make the score cards for each of the desired metric and variable
        for metric in metrics_to_plot:
            ...
            for variable in variables_to_plot:
                ...
                make_score_cards(clim_list['%s_%s'%(metric, variable)], metric_list['%s_%s'%(metric, variable)],
                                 months, variable = variable, score = metric, subset = args.subset,
                                 cmin = cmin, cmax = cmax, cint = cint, 
                                 path = args.figure_path, savename = savename)

    if args.make_variation_plots:
        # Load in data peacemeal, final format needs to be time for true labels, and fh x time for pred
        ...
        for direct in tqdm(all_direct):
            ...
            for file in np.sort(nc_files):
                ...
                with Dataset(file, 'r') as nc:
                    ...
                    for var in variables_to_plot:
                        ...
                        #     # Spatial average to shape is (forecasts)
                        #     sub_data = np.nanmean(sub_data, axis = -1)
                        #     sub_data = np.nanmean(sub_data, axis = -1)
            ...
            #         pred_datasets[sub][var].append(np.nanmean(tmp, axis = 0))

        for var in variables_to_plot:
            for sub in subsets:
                true_datasets[sub][var] = np.concatenate(true_datasets[sub][var])
                ...
                print('True: ', true_datasets[sub][var])
                print('Pred: ', pred_datasets[sub][var])
                
                # Make savename, and plots
                savename = 'violin_plot_%s_%s.png'%(sub, var)
                plot_violin(true_datasets[sub][var], pred_datasets[sub][var].T, 
